...
def show_plots(root_path: Path):
    <PUSSY1>plotting.util import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'lotus plots add -d [dir]' and remove with"
        + " 'lotus plots remove -d [dir]'"
        + " Scan and check plots with 'lotus plots check'"
    )
    print()
...
def check_cmd(
    ...
    <PUSSY1>plotting.check_plots import check_plots
...
def remove_cmd(ctx: click.Context, final_dir: str):
    <PUSSY1>plotting.util import remove_plot_directory
